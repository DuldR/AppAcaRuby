...
class SuperComputerPlayer < ComputerPlayer
  def move(game, mark)
    ...
    # test_node = TicTacToeNode.new(game.board, mark)

    # pot_moves = test_node.children.select { |node| node.board.winner == mark }

    # pick_move = pot_moves.find { |child| child.winning_node?(mark) }
    # return pick_move.prev_move_pos if pick_move

    # pick_move = pot_moves.find { |child| !child.losing_node?(mark) }
    # return node.prev_move_pos if pick_move
